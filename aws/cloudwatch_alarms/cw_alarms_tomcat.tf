resource "aws_cloudwatch_metric_alarm" "tomcat_process_status_check" {
  for_each                  = local.all_ec2
  alarm_description         = "This metric monitors tomcat process status check."
  alarm_name                = "${each.key} EC2 TomcatStatusCheckFailed"
